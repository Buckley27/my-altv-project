...
const weapons = [
  "dagger", "bat", "bottle", "crowbar", "flashlight", "golfclub", "hammer", "hatchet", "knuckle", "knife", "machete", "switchblade", "nightstick", "wrench", "battleaxe", "poolcue", "stone_hatchet", "pistol", "pistol_mk2", "combatpistol", "appistol", "stungun", "pistol50", "snspistol", "snspistol_mk2", "heavypistol", "vintagepistol", "flaregun", "marksmanpistol", "revolver", "revolver_mk2", "doubleaction", "raypistol", "microsmg", "smg", "smg_mk2", "assaultsmg", "combatpdw", "machinepistol", "minismg", "raycarbine", "pumpshotgun", "pumpshotgun_mk2", "sawnoffshotgun", "assaultshotgun", "bullpupshotgun", "musket", "heavyshotgun", "dbshotgun", "autoshotgun", "assaultrifle", "assaultrifle_mk2", "carbinerifle", "carbinerifle_mk2", "advancedrifle", "specialcarbine", "specialcarbine_mk2", "bullpuprifle", "bullpuprifle_mk2", "compactrifle", "mg", "combatmg", "combatmg_mk2", "gusenberg", "sniperrifle", "heavysniper", "heavysniper_mk2", "marksmanrifle", "marksmanrifle_mk2", "rpg", "grenadelauncher", "grenadelauncher_smoke", "minigun", "firework", "railgun", "hominglauncher", "compactlauncher", "rayminigun", "grenade", "bzgas", "smokegrenade", "flare", "molotov", "stickybomb", "proxmine", "snowball", "pipebomb", "ball"
];

const playerSpawns = new Map(); // Map to store player-specific spawn points
...
alt.on("playerConnect", (player) => {
  ...
  player.spawn(0, 0, 72, 0); // Default spawn point
...
alt.on("playerDeath", (player, killer, weapon) => {
  const spawn = playerSpawns.get(player.id) || { x: 0, y: 0, z: 72 }; // Use saved spawn or default
...
alt.on("playerDisconnect", (player, reason) => {
  ...
  alt.log(`${player.name} has leaved the server because of ${reason}`);
});
...
// New /setspawn command
chat.registerCmd("setspawn", (player, args) => {
  const { x, y, z } = player.pos;
  playerSpawns.set(player.id, { x, y, z });
  chat.send(player, `Spawn point set to (${x}, ${y}, ${z})`);
});
